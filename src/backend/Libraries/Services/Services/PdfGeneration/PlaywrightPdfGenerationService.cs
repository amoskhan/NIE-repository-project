...
{
    // Logo embedded by Services.csproj from src/frontend/main/src/assets/app-logo.svg.
    private const string ReportLogoResourceName = "Services.Services.Reports.ReportLogo.svg";

    // Number / date / currency formatting for rendered reports. Matches the
    // "Application:Locale" default shipped in appsettings.json — change both together.
    private static readonly CultureInfo ReportCulture = CultureInfo.GetCultureInfo("en-SG");
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            ("Total Orders", totalOrders.ToString(ReportCulture)),
        ...
        html.Append(RenderTable(
            ...
            {
                row.Status.ToString(),
                row.Count.ToString(ReportCulture),
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RenderTable(
            ...
            {
                row.VendorId.ToString(ReportCulture),
                row.VendorName,
                row.Orders.ToString(ReportCulture),
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RenderTable(
            ...
            {
                row.Stage.ToString(),
                row.Actions.ToString(ReportCulture),
    ...
    private string BuildReportDocument(string title, string content, ReportRequestDto filters)
    {
        ...
        var logoDataUri = TryGetLogoDataUri();
            ...
            pagesHtml.Append(CultureInfo.InvariantCulture, $"""
                <article class="report-page">
                  ...
                  <footer class="report-footer">
                    ...
                    <span class="page-counter">Page {pageNumber.ToString(ReportCulture)} of {totalPages.ToString(ReportCulture)}</span>
                    <span>Generated {Html(generatedAt.ToString("dd MMM yyyy HH:mm:ss", ReportCulture))} ({Html(DateTimeHelper.TimeZoneId)})</span>
                  </footer>
                </article>
                """);
        }
    ...
    private static string RenderFilterSummary(ReportRequestDto filters)
    {
        var entries = new List<(string Label, string? Value)>
        {
            ...
            ("Vendor", filters.VendorId?.ToString(ReportCulture)),
    ...
    /// <summary>
    /// Renders the report header brand: the embedded logo when it is available,
    /// otherwise a plain text mark so the header never renders empty.
    /// </summary>
    private static string RenderLogo(string? logoDataUri)
    {
        return string.IsNullOrWhiteSpace(logoDataUri)
            ? "<span class=\"brand-mark\">APP</span>"
            : $"<img src=\"{Html(logoDataUri)}\" alt=\"App Template\">";
    }

    /// <summary>
    /// Reads the report logo that <c>Services.csproj</c> embeds from
    /// <c>src/frontend/main/src/assets/app-logo.svg</c> and returns it as a data URI.
    /// Returns null (and logs) when the resource is missing, so reports still render.
    /// </summary>
    private string? TryGetLogoDataUri()
    {
        try
        {
    ...
    private static string FormatCurrency(decimal amount) =>
        amount.ToString("C", ReportCulture);

    private static string FormatDate(DateTime date) =>
        date.ToString("dd MMM yyyy", ReportCulture);

    private static string FormatDate(DateOnly date) =>
        date.ToString("dd MMM yyyy", ReportCulture);

    private static string FormatDateTime(DateTime date) =>
        date.ToString("dd MMM yyyy HH:mm", ReportCulture);
