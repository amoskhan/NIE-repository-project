...
public class AuditLogger : IAuditLogger
{
    ...
    public async Task LogLoginAsync(string userId, string userName)
    {
        ...
        entry.NewValues = JsonSerializer.Serialize(new { userId, userName, loginTime = Shared.Helpers.DateTimeHelper.Now });
        await SaveEntryAsync(entry);
    }
    ...
    private async Task SaveEntryAsync(AuditLog entry)
    {
        try
        {
            ...
            await _context.Database.ExecuteSqlRawAsync(
                ...
                Shared.Helpers.DateTimeHelper.AsUnspecified(entry.Timestamp),
