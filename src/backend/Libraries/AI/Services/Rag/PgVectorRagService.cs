...
public class PgVectorRagService : IPgVectorRagService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return results.Select(r => new RagSearchResult
        {
            ...
            SourceId = r.SourceId.ToString(System.Globalization.CultureInfo.InvariantCulture),
        }).ToList();
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!int.TryParse(sourceId, System.Globalization.CultureInfo.InvariantCulture, out var numericSourceId))
        {
            _logger.LogWarning(
                "Skipping RAG indexing — sourceId {SourceId} is not numeric (ChatEmbedding.SourceId is int).",
                sourceId);
            return;
        }
        ...
        var existing = await _db.Set<ChatEmbedding>()
            .Where(e => e.SourceType == sourceType && e.SourceId == numericSourceId)
            .ToListAsync(cancellationToken);
        if (existing.Count > 0)
        {
