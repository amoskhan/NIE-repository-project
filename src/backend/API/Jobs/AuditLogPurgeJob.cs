...
{
    private const string MonitorSlug = "apptemplate-audit-log-purge";
    ...
    private static void ConfigureSentryMonitor(Sentry.SentryMonitorOptions options)
    {
        options.Interval("0 2 * * *");
        options.CheckInMargin = TimeSpan.FromMinutes(30);
        options.MaxRuntime = TimeSpan.FromMinutes(60);
        options.FailureIssueThreshold = 1;
        options.RecoveryThreshold = 1;
        // Same zone the TickerQ scheduler runs the cron against, so Sentry's
        // "missed check-in" window lines up with when the job actually fires.
        options.TimeZone = Shared.Helpers.DateTimeHelper.IanaTimeZoneId;
    }
}
