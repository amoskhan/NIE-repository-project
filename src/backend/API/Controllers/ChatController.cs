...
public class ChatController : BaseController
{
    ...
    [HttpPost("conversations/{id:int}/rename")]
    [RequireAccessFunction(AccessFunctionCodes.Api.ChatUse)]
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            Response.StatusCode = StatusCodes.Status400BadRequest;
            await Response.WriteAsync($"Message exceeds maximum length of {MaxMessageLength} characters", cancellationToken);
            return;
        }
        ...
        try
        {
            ...
            await WriteSseAsync(
                new AIStreamEventDto { Type = "done" },
                cancellationToken);
        }
        ...
        {
            await WriteSseAsync(
                new AIStreamEventDto { Type = "error", Error = ex.Message },
                cancellationToken);
        }
        catch (KeyNotFoundException)
        {
            await WriteSseAsync(
                new AIStreamEventDto { Type = "error", Error = "Conversation not found." },
                cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            await WriteSseAsync(
                new AIStreamEventDto { Type = "error", Error = "The assistant could not respond right now." },
                cancellationToken);
        }
    }
    ...
    private static DateTimeOffset ToResponseDateTime(DateTime value)
    {
        ...
        return new DateTimeOffset(localValue, DateTimeHelper.TimeZone.GetUtcOffset(localValue));
    }
}
...
{
    [Required]
    [RegularExpression("^(thumbs_up|thumbs_down)$")]
