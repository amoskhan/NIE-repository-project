...
export function normalizeReportPageSetup(
  ...
  const defaultFormat =
    normalizeReportPageFormat(pageSetup?.defaultFormat) ??
    DEFAULT_REPORT_PAGE_SETUP.defaultFormat;
  const defaultOrientation =
    normalizeReportPageOrientation(pageSetup?.defaultOrientation) ??
  ...
  return {
    ...
    defaultOrientation,
    allowFormatChange:
      pageSetup?.allowFormatChange ??
      DEFAULT_REPORT_PAGE_SETUP.allowFormatChange,
    allowOrientationChange:
      pageSetup?.allowOrientationChange ??
      DEFAULT_REPORT_PAGE_SETUP.allowOrientationChange,
    formats: formats.includes(defaultFormat)
      ? formats
...
function normalizeReportPageOrientations(
  ...
  const normalized = (orientations ?? REPORT_PAGE_ORIENTATIONS)
    ...
    .filter(
      (orientation): orientation is ReportPageOrientation =>
        orientation !== null,
    );
